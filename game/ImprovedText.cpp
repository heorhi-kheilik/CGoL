...
sf::Vector2f ImprovedText::calculateStartPosition() {
    ...
    position.y = (alignRectangle_.top +
                  ceil((alignRectangle_.height + getFont()->getGlyph('x', getCharacterSize(), false).bounds.height) / 2) -
                  getCharacterSize()
                  );
    return position;
}
...
sf::Vector2f ImprovedText::calculateStartPosition(sf::Rect<int> alignRectangle) {
    ...
    position.y = (alignRectangle.top +
                  ceil((alignRectangle.height + getFont()->getGlyph('x', getCharacterSize(), false).bounds.height) / 2) -
                  getCharacterSize());
    return position;
}
